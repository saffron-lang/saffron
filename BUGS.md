...
#146 is the alias/type-re-export fix (`var X = Module.SomeType`), also under
`## Resolved`. It was written on the ide-stage0-spans worktree and renumbered
three times at merge — first to #143, then to #145 when origin/main's own #143
(non-`Bool` condition) landed, then to #146 when origin/main's #145 (call-site
argument checking) landed. The lesson below about reading the next-free number
from `main` is exactly why.
...
### 146. FIXED — `var X = Module.SomeType` looked like a type re-export, emitted a call to a symbol nothing defines, and forced every signature in `@ast` to `Any`
...
**Renumbered #142 → #143 → #145 → #146 across three merges.** This entry was
...
origin/main's #145 (call-site argument checking) landed at the third merge, so it
takes #146 — the next free number on `main`. Thrice bitten by the same
per-worktree collision the note at the top of `## Open` warns about: read the
next-free number from `main`, never from a worktree.
