...
**Update (2026-08-02): shipped as Phase 5b.** Both the block and the gate are behind us; see the Phase 5b entry in §7 for what landed. The diagnosis stays because it is the reason this modifier came fourth rather than first, and because it is the one place in this design where a representation defect made a *semantics* unimplementable rather than merely awkward.
...
- `internal` already reports `'internal' is not implemented yet; use 'private' or 'public'` from Phase 4's parser — a real diagnostic with a span, not a silent accept. That is the right placeholder for Phase 5 to replace. (Both placeholders are now gone: Phase 5 replaced `internal`'s, and Phase 5b replaced `protected`'s with real semantics plus a positional legality error at top level.)
...
**Phase 5's enforcement is now done.** A single registration path, `register_module_decl`, records every top-level declaration's owning module (`decl_module_file`), owning package (`decl_module_package`) and — new — its non-public visibility in `decl_visibility`, keyed by the qualified name so the three tables cannot drift. Public is never stored: an absent key means "nothing to enforce", so the common case is one `has`. `ClassDecl` records visibility directly rather than through `register_module_decl`, because `register_class_identity` already owns its two identity maps and routing through the shared path would double-write them. All five top-level kinds are covered — the `FunDecl`, `EnumDecl`, `VarDecl`, `TypeAlias` and `ClassDecl` arms of `register_decl` — and `type Name = T` is covered in the `VarDecl` arm, because the parser desugars it to a `VarDecl` with docstring `"@type_alias"` and never emits `TypeAlias` (the same finding Phase 4 recorded for the leak pass).

One decision function, `check_module_ref_visibility(name, slot)`, decides all three access paths, because they converge on the same fact — a resolved reference carrying the DEFINING module's prefix:

- **path 3 (bare cross-module)** — `resolve.sf` stamps `Ref.slot` with the defining module's prefix, so a called bare `helper()` is decided in `infer_call` (which reads the callee name directly and never routes the callee through `infer_type`'s `Ref` arm, so the check had to be added in both places) and a bare *read* is decided in the `Ref` arm.
- **path 1 (named import)** — collapses onto path 3 at the checker: resolve binds the imported name and stamps the same defining prefix, so the bare-reference check catches it with no separate arm.
- **path 2 (alias-qualified `Mod.name`)** — arrives as a `MethodCall` whose receiver is a module alias the checker types as `Any`. The entry file's `resolved_aliases` map (already built for codegen) is threaded into the checker as `entry_aliases`, which recovers the defining prefix so the same decision runs. This is the path §6a flags as easiest to leave open, and it is the one that needs the extra plumbing.

`private` compares the defining prefix against `current_prefix` (same file ⇒ allowed, including a same-file free function — the deliberate difference from class-private). `internal` compares `decl_module_package` against `current_package`, and a packageless marker on either side **denies** rather than grants — the same conservative posture as `same_package`. The parser now accepts `internal` (it was a Phase 4 placeholder diagnostic); `protected` is Phase 5b, below.

**Verified against the built compiler**, all three paths for `private`, both linkable forms for `internal`, in detached worktrees: same-file use of a module's own private/internal names is allowed; cross-module `private` is refused on all three paths; same-package `internal` is allowed (alias and named-import forms link and run), cross-package and packageless `internal` are refused. Each fail test was proven to depend on the modifier by stripping it and confirming the file then compiles clean. One pre-existing codegen limitation surfaced and was confirmed on `origin/main` too: a bare call to a **directly-imported `.sf` file's** function lowers to an undefined LLVM symbol (the named-import and alias-qualified forms link fine). It is unrelated to visibility, so pass tests use the linkable forms and the bare/named-import paths are exercised by fail tests, which stop at the checker before codegen.

Phase 4's `test/fail/visibility_internal_unimplemented.sf` — which pinned that the compiler *refuses* `internal` while its semantics were missing — is now obsolete by construction, and became `test/pass/visibility_internal_flat.sf` rather than being deleted. Its source is unchanged; only the expected verdict flipped, which is the honest record of what this phase changed. Kept as a pass test because a flat single-file program is its own path: with no module boundaries, the declaring and using sides are both the entry module, so it pins the same-prefix early exit that keeps a program's own `internal` name legal against itself. Full-suite failure name sets are identical to the `a2e9c08` baseline (25 pre-existing failures, no additions), with a `[STAGE 2]` fixed-point bootstrap and gen2 untouched at blob `ebd4844`.

**Phase 5b — `protected`. DONE.** The fourth and last modifier, and the smallest slice of the four, because Phase 3b had already done the hard part.

`protected` is class-`private` **plus one clause**: a subclass of the declaring class may also access the member. That is the entire semantic difference, and it is why Phase 5b added no second table, no second ancestry walk and no second checking function. Phase 4's `private_members: Map<String, String>` already stored a value that was always the constant `"private"`; `protected` was implemented by **giving that value meaning** — the map became `member_visibility`, `private_declaring_class` finds either modifier in one walk, and `check_member_visibility` decides which by reading the stored word. A parallel `protected_members` table was the alternative and is rejected: its keys are built from a generic base name and a qualified class key, and two tables keyed that way can drift out of step in a way one table cannot.

Four decisions worth recording, since each could reasonably have gone the other way:

- **`internal` is not a member restriction.** `is_restricting_visibility` answers true for exactly `private` and `protected`. On a member, `internal` would mean package scope, which is not part of Kotlin parity as scoped here (§6a covers `internal` at the top level only), so treating it as restricting would enforce a rule this design never specified. It parses, it is recorded at the top level, and on a member it means public — the same reading Phase 4 gave it.
- **`protected` is an error at top level**, per §1's table, and that is the one positional legality rule in the parser: `take_visibility` now takes an `in_class_body` flag. Accepting it would compile a declaration whose stated intent the compiler ignores, which is the same wrong answer the `internal` placeholder existed to avoid. The other three modifiers are legal in both positions and differ only in *meaning*, which the checker resolves because only the checker knows which position it is looking at.
- **A `protected` abstract method is a contradiction**, and for a narrower reason than the `private` case it extends. `protected` *does* reach subclasses, so a protected abstract method looks implementable. It is not: the modifier restricts who may **call** the name, and an abstract method exists to be called by code holding the interface type, which by construction is not a subclass. Kotlin agrees — `protected abstract` is legal on a class but not as an interface requirement. Saffron has one declaration form for both, so bodyless is the available discriminator.
- **`protected init` is `private init` plus subclasses**, keeping the two **monotone** on constructors: everything `private init` permits, `protected init` permits too, module-scoped factory included. Kotlin's `protected` is not a superset of its `private` on the file axis, but that axis is where this constructor rule already departs from Kotlin deliberately (§6a, for the factory pattern), and making `protected` the *narrower* of the two there would be a trap — an author widening `private init` to reach subclasses would silently lose the same-module factory that was the reason to restrict the constructor at all.

One latent defect fell out of the work rather than being sought: the §6b leak pass runs on everything that is not `private`, but labelled every subject `"public X 'Owner.member'"`. That was already wrong for `internal` before `protected` existed — calling a `protected` field "public" in a diagnostic sends the reader looking for a `public` they will not find, and invites the conclusion that the check is wrong rather than that their field leaks. `exposure_label` now names the modifier that actually applies, threaded through all seven subject labels.

**Verified against the built compiler**, each semantic probed individually and each fail test proven to depend on its modifier by stripping it and confirming the file then compiles clean: a `protected` member is reachable from its own class body, from a subclass, and from a subclass reaching it through a **secondary** base — the case Phase 3b unblocked, since `inherits_from` now searches the whole inheritance DAG. Replacing `protected` with `private` in that same file makes the secondary-base access an error, which is what proves the added clause is load-bearing rather than decorative; replacing it with `internal` compiles clean, which proves the `is_restricting_visibility` gate. A `protected` member is refused from an unrelated class that merely *holds* an instance — deliberately a class rather than a free function, since a free function is already refused by plain class-private and would pass even if the clause said "allow any class."

**One real defect was found and fixed during this phase, and it is worth reading §11 decision 2's amended entry for the detail.** Answering visibility by searching the ancestry for a *restricting* class refuses accesses that dispatch resolves to a different, unrestricted class — measurable, not theoretical, and it is the exact breaking-change outcome §11 decision 2 rejects, reached by a different route. Visibility now answers from the class dispatch picks, and dispatch turned out to be depth-first pre-order rather than breadth-first. Two tests pin it (`protected_tie_dispatch_order.sf`, `protected_tie_earlier_base.sf`). The general lesson generalises past `protected`: when a check and the code generator both need to know "which declaration wins", they must read it from one place (M1), and "same order" is not the same claim as "same traversal".
...
**Phase 8+ — deferred features.** ~~Widen `ClassDecl.parent` to `List<String>`, which unblocks `protected` and closes L7's coverage hole.~~ **Done — it became Phase 3b and landed 2026-08-02** (BUGS #111): it is independent of visibility, and multiple inheritance was not merely unsound for `protected` but broken outright, so it was worth doing on its own account. What remains deferred here is L7's coverage hole — `protected` itself shipped as Phase 5b.
...
- `protected_from_unrelated_class.sf` — a `protected` member read from a class that is not a subclass (§5b). **Landed**, and deliberately a class rather than a free function: a free function is already refused by plain class-private, so it would pass even if the subclass clause were mistakenly written as "allow any class".
- `protected_top_level.sf` — `protected` on a top-level declaration, per §1's table. Not in this plan's first draft; added because it is the one *positional legality* rule in the parser and the other three modifiers share no such rule, so nothing else would cover it.
- `protected_abstract.sf` — a bodyless `protected` method. Also added late: without it the `protected` branch of the abstract-contradiction check is unreachable by any test, and a later "simplification" back to `vis == "private"` would look harmless.
- `protected_init_external.sf` — a `protected init` constructed from a module that is not a subclass. The boundary that keeps the monotonicity clause below from being a blanket allowance.
...
- ~~`protected_from_subclass_of_secondary_base.sf`~~ — **landed as `protected_from_subclass.sf`**, merged with the same-class and `protected init` factory cases rather than split across three files, because all three are positive assertions about one modifier and a single file keeps them side by side where the contrast is legible. It carries the secondary-base case this line asked for (a `protected` default method on `parents[1]`, read from the subclass — what Phase 3b unblocked), the same-class case, and the same-module factory that pins constructor monotonicity. Renamed because the file is no longer only about the secondary base.
...
- ~~`protected`~~ — **in scope and now shipped**, last of the four, as Phase 5b (§7). It was blocked on `ClassDecl.parent: String` and the parser discarding parents 2..n; that block was removed by BUGS #111, and the widening's promotion gate closed at `332aafb`.
...
2. **`protected` and multiple inheritance** → **follow the rule the tree already has: earlier bases win ties, in declaration order, and abstract names do not claim a tie.**
   ...
   **Implemented in Phase 5b, and the implementation is not what "follow the existing rule" naively suggests.** Writing the visibility walk to *find the first restricting ancestor, in declaration order* looks like it follows this decision and does not: it answers from a different class than dispatch does. Measured on the built compiler — given `interface A { protected fun tag() }`, `interface B { fun tag() }`, `class C extends Holder, B, A` — `C().tag()` runs **B's** body and returns 2, while the "first restricting ancestor" search refused the call citing `A.tag`. That is a diagnostic about a modifier on a declaration whose body never executes, unsilenceable by any edit at the call site, and it is the breaking-change outcome this decision explicitly rejects, arrived at by a different route.

   The rule as implemented is therefore sharper than the sentence above: **visibility is answered by the class the dispatch picks.** `private_declaring_class` stops at the first ancestor that *declares* the name and reports whether that one restricts it. Dispatch order is what actually runs, so it is not negotiable, and it turns out to be **depth-first pre-order** in declaration order rather than breadth-first — verified with an `A extends A2` chain, where `extends Holder, A, B` runs A2's body, not B's. The walk matches. `test/pass/protected_tie_dispatch_order.sf` pins both the tie and the depth ordering; `test/fail/protected_tie_earlier_base.sf` pins the reverse order, where the dispatch winner *is* the restricted base and the access must be refused.
