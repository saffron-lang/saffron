...
**Phase 8+ — deferred features.** ~~Widen `ClassDecl.parent` to `List<String>`, which unblocks `protected` and closes L7's coverage hole.~~ **Landed 2026-08-02, ahead of every phase above it** (BUGS #104): it is independent of visibility, and multiple inheritance was not merely unsound for `protected` but broken outright, so it was worth doing on its own account. Its promotion gate is therefore the first one in this plan to close, not the last. What remains deferred here is `protected` itself and L7's coverage hole.
...
- **`protected`** — was blocked on `ClassDecl.parent: String` and the parser discarding parents 2..n. **That block is removed** (BUGS #104): the checker can now resolve a base at any position, so `protected` is implementable. It is still not in this slice, and it still waits on the widening's own promotion gate.
